...
// Copy of https://github.com/google/flatbuffers/blob/master/dart/lib/src/reference.dart
...
class Reference {
  ...
  Reference operator [](Object key) {
    if (key is int && ValueTypeUtils.isAVector(_valueType)) {
      ...
      final reference = Reference._(_buffer, elementOffset,
          BitWidthUtil.fromByteWidth(_byteWidth), 0, '$_path[$index]');
      reference._byteWidth = 1;
      if (ValueTypeUtils.isTypedVector(_valueType)) {
        reference._valueType =
            ValueTypeUtils.typedVectorElementType(_valueType);
        return reference;
      }
      if (ValueTypeUtils.isFixedTypedVector(_valueType)) {
        reference._valueType =
            ValueTypeUtils.fixedTypedVectorElementType(_valueType);
        return reference;
      }
      final packedType =
          _buffer.getUint8(_indirect + length * _byteWidth + index);
  ...
  int get length {
    ...
    if (ValueTypeUtils.isFixedTypedVector(_valueType)) {
      ...
      var size = _readUInt(
          indirect - sizeByteWidth, BitWidthUtil.fromByteWidth(sizeByteWidth));
